...
| 9 | Reference-performance-environment ownership | **Infrastructure decision w/ provisional default** | *Provisional:* a **Linux amd64 GitHub Actions runner + committed `perf/reference-v1.json`** baseline, advisory-only initially; a dedicated bare-metal runner is a *later* SRE decision, not a blocker. See §12. | **No** (relative benchmarking proceeds now) |
