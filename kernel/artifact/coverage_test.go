...
// TestGenerateVersionConflict deterministically forces a unique-violation on
// (tenant, kind, version): a first transaction inserts version 1 but holds open
// (uncommitted); a second transaction — under READ COMMITTED — computes the same
// next version, blocks on the unique index, then fails with 23505 when the first
// commits. That drives isUniqueViolation → true and the KindConflict branch.
func TestIntegrationGenerateVersionConflict(t *testing.T) {
	h := testkit.NewDB(t)
	p := artifact.New(model.UUIDv7())
	ctx := actx(uuid.New())
	...
	go func() {
		...
		holderErr = h.TxM.WithTenant(ctx, func(ctx context.Context, db database.TenantDB) error {
			...
			<-release       // hold the transaction open so the second collides
			return nil
		})
	}()
	...
	// Second generate: its next-version subquery runs while row 1 is uncommitted,
	// so it also targets version 1 and parks on the unique index.
	conflictCh := make(chan error, 1)
	go func() {
		conflictCh <- h.TxM.WithTenant(ctx, func(ctx context.Context, db database.TenantDB) error {
			_, e := p.Generate(ctx, db, artifact.Input{Kind: "race", Content: []byte("second")})
			return e
		})
	}()

	// Give the second transaction time to reach and block on the INSERT, then let
	// the holder commit so the blocked INSERT resolves to a unique violation.
	time.Sleep(300 * time.Millisecond)
	close(release)

	conflictErr := <-conflictCh
	wg.Wait()
	...
	if conflictErr == nil {
		t.Fatal("second generate must fail with a version conflict")
	}
	if kerr.KindOf(conflictErr) != kerr.KindConflict {
		t.Fatalf("kind = %v, want KindConflict; err = %v", kerr.KindOf(conflictErr), conflictErr)
	}
	if e, ok := kerr.As(conflictErr); !ok || e.Code != "version_conflict" {
		t.Fatalf("code = %+v, want version_conflict", e)
	}
}
