...
// Transition, or SLA) must never alter the validated graph running
...
func TestDefinitionNestedDataIsNotAliased(t *testing.T) {
	...
	def := Definition{
		...
		Steps: map[string]Step{
			"review": {
				...
				SLA:       &SLA{Due: "P1D", RemindAfter: "PT1H"},
	...
	if review.SLA.Due != "P1D" {
		t.Fatalf("retained alias changed the SLA: %+v", review.SLA)
	}
