...
func TestUnitFakeVerifier(t *testing.T) {
	...
	if err := v.Verify("", nil, map[string]string{"X-Test-Sig": "open-sesame"}); err != nil {
		t.Fatalf("matching header should pass, got %v", err)
	}
	err := v.Verify("", nil, map[string]string{"X-Test-Sig": "wrong"})
...
func TestUnitHMACVerifier_MissingHeader(t *testing.T) {
	err := webhook.HMACVerifier{}.Verify(testSecret, []byte(`{}`), map[string]string{})
	if kerr.KindOf(err) != kerr.KindUnauthenticated {
