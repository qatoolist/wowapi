...
func TestTOTP_WrongCodeRejected(t *testing.T) {
	...
	now := time.Unix(1_700_000_000, 0).UTC()
	ok, err := mfa.VerifyTOTPAt(secret, "000000", now, mfa.TOTPOptions{})
	if err != nil {
		t.Fatalf("VerifyTOTPAt: %v", err)
	}
	if ok {
		real, _ := mfa.TOTPCodeAt(secret, now, mfa.TOTPOptions{})
		if real == "000000" {
			t.Skip("generated secret coincidentally produced 000000; flaky by construction, skip")
		}
