...
func TestBackoff(t *testing.T) {
	...
	for attempt, want := range cases {
		if got := webhookBackoff.Next(attempt); got != want {
			t.Fatalf("webhookBackoff.Next(%d) = %v, want %v", attempt, got, want)
		}
	}
}
...
func TestDedupExtID(t *testing.T) {
	env := Envelope{EventID: "evt-42", CanonicalBody: []byte(`{"a":1}`)}
	if got := dedupExtIDEnv(env); got != "evt-42" {
		t.Fatalf("want provider id evt-42, got %q", got)
	}

	// Id-less: synthesized from the raw body as "sha256:<hex>".
	body := []byte(`{"b":2}`)
	idless := Envelope{CanonicalBody: body}
	sum := sha256.Sum256(body)
	want := "sha256:" + hex.EncodeToString(sum[:])
	if got := dedupExtIDEnv(idless); got != want {
		t.Fatalf("synthetic id mismatch\n got  %s\n want %s", got, want)
	}

	// Same body → same synthetic id (stable/deterministic).
	if dedupExtIDEnv(idless) != dedupExtIDEnv(Envelope{CanonicalBody: []byte(`{"b":2}`)}) {
		t.Fatal("synthetic dedup id is not stable for identical bodies")
	}
	// Different body → different id.
	if dedupExtIDEnv(idless) == dedupExtIDEnv(Envelope{CanonicalBody: []byte(`{"b":3}`)}) {
