...
// New wires the service. registry/store/outbox/idgen are required; authz and
// hooks are optional.
...
// InitiateUpload reserves the next version number and returns a presigned PUT for
// a tenant-prefixed key. The version row is written only on ConfirmUpload.
func (s *Service) InitiateUpload(ctx context.Context, db database.TenantDB, docID uuid.UUID) (UploadSession, error) {
	...
	if err := db.QueryRow(ctx,
		`SELECT COALESCE(MAX(version_no),0)+1 FROM document_versions WHERE document_id = $1`, docID).Scan(&next); err != nil {
		return UploadSession{}, kerr.Wrapf(err, "document.InitiateUpload", "next version")
	}
	...
	url, err := s.store.PresignPut(ctx, key, s.putTTL)
	if err != nil {
		return UploadSession{}, kerr.Wrapf(err, "document.InitiateUpload", "presign put")
	}
	return UploadSession{DocumentID: docID, VersionNo: next, StorageKey: key, Upload: url}, nil
}
...
func (s *Service) ConfirmUpload(ctx context.Context, db database.TenantDB, in ConfirmInput) (uuid.UUID, error) {
	...
	_, err = db.Exec(ctx,
		...
		verID, in.DocumentID, in.VersionNo, in.StorageKey, mime, info.Size, strings.ToLower(in.DeclaredChecksum), actor)
	...
	if _, err := db.Exec(ctx,
		...
		in.DocumentID, in.VersionNo, actor); err != nil {
		return uuid.Nil, kerr.Wrapf(err, "document.ConfirmUpload", "bump document version")
	}
	if err := s.emit(ctx, db, "document.version_added", docRef(in.DocumentID), map[string]any{"version_no": in.VersionNo, "mime": mime}); err != nil {
		return uuid.Nil, err
	}
	return verID, nil
}
