...
6 deferred items (DEF-01..DEF-06), each with an explicit reopen trigger or prerequisite, none
assigned to a wave in this programme. DX-03 (module DSL design) is a partial exception — it has a
nominal target `W06-E01-S001` for a design-investigation story only; the DSL build-out itself
remains deferred. See `requirement-traceability-matrix.md` exclusion notes.
