...
func (f *faultyStore) PresignPutChecksum(ctx context.Context, key, checksum string, ttl time.Duration) (storage.PresignedURL, error) {
	...
	return f.Memory.PresignPutChecksum(ctx, key, checksum, ttl)
}
...
func TestIntegrationConfirmUploadDuplicateVersion(t *testing.T) {
	...
	// Tx1: create + reserve two sessions. With the per-document counter they
	// receive distinct version numbers (1 and 2) even though no version is
	// committed yet.
	...
	if s1.VersionNo != 1 || s2.VersionNo != 2 {
		t.Fatalf("sessions must reserve versions 1 and 2: s1=%d s2=%d", s1.VersionNo, s2.VersionNo)
	}
	...
	// Tx3: confirm s2 as version 2 also succeeds — the counter fix eliminates
	// the duplicate-version false conflict.
	if err := a.h.TxM.WithTenant(a.ctx, func(ctx context.Context, db database.TenantDB) error {
		_, e := a.svc.ConfirmUpload(ctx, db, document.ConfirmInput{
			SessionID: s2.SessionID, DocumentID: docID, VersionNo: s2.VersionNo, StorageKey: s2.StorageKey,
			DeclaredSize: int64(len(body)), DeclaredChecksum: sum(body), DeclaredMIME: "text/plain",
		})
		return e
	}); err != nil {
		t.Fatalf("second confirm must succeed with counter allocation, got %v", err)
	}

	// Both versions are present.
	var count int
	if err := a.h.Admin.QueryRow(context.Background(),
		`SELECT count(*) FROM document_versions WHERE document_id=$1`, docID).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 2 {
		t.Fatalf("want 2 confirmed versions, got %d", count)
	}
}
...
func TestIntegrationSweepMultipleDocsAndReRun(t *testing.T) {
	...
	if err := a.h.TxM.WithTenant(a.ctx, func(ctx context.Context, db database.TenantDB) error {
		...
		a.store.Put(sess.StorageKey, body)
		_, e = a.svc.ConfirmUpload(ctx, db, document.ConfirmInput{
			SessionID: sess.SessionID, DocumentID: d1, VersionNo: sess.VersionNo, StorageKey: sess.StorageKey,
			DeclaredSize: int64(len(body)), DeclaredChecksum: sum(body), DeclaredMIME: "text/plain",
		})
		return e
	}); err != nil {
