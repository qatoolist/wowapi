...
| CA-7 traceability | P0 | **CLOSED (status honesty)** | `hardening-plan.md` STATUS honest; this closure matrix (§6); ROADMAP-wowapi remediation pointer + de-linked out-of-repo product ROADMAP.md; CHANGELOG `[Unreleased]` CA-1..CA-15 entry (verified honest — the "end-to-end OTel" phrasing was only a commit message, not the CHANGELOG body). Residual: `progress.md` still stops at Phase 12 (superseded by this matrix). |
...
| CA-9 O1 async propagation | P1 | **CLOSED** | `events_outbox.trace_context` (migration 00024, reversible); the outbox writer injects the request's W3C traceparent, the relay extracts it and dispatches each event under a child span (`WithWriterTracer`/`WithRelayTracer`, wired via `kernel.Deps.Tracer` in both generated mains). Test: an injected carrier is stored on the event and extracted by the relay on dispatch. Residual: jobs/notify sub-paths reuse the same tracer seam (outbox is the primary async fan-out). |
...
| CA-11 audit completeness | P1 | **CLOSED (all but anchor-export)** | `module.Context` accessors for `Audit`/`Sequence`/`Bulk`/`Artifacts`; `audit_logs.tx_id` (migration 00023, reversible, correlation test); `wowapi audit verify` CLI (verified end-to-end incl. tamper detection). **Remaining:** scheduled anchor-export (offline tail-truncation detection) — small, low risk. |
| CA-12 O2/O3/O5 finishers | P1 | **PARTIAL** | **CLOSED:** O3 published upgrade/deprecation policy (`docs/operations/upgrade-and-deprecation-policy.md`). **Remaining:** O2 schema-snapshot diffing in the reversibility drill; O5 scripted PITR/object-storage restore legs (or rescope O5 to a staging drill). Low risk (operational polish). |
...
**Exit gate:** P0 corrective actions **CA-1…CA-5 and CA-7 are CLOSED and verified**; **CA-6** (hosted CI)
is written but **cannot be proven green without a push** (the one remaining P0 blocker, and it is
process-not-code). P1: **CA-8/9/11/13/14/15 CLOSED** (CA-11 all but a small anchor-export); **CA-12**
O2/O5 remain OPEN, **CA-10** is a recorded deployment-concern rescope — all low-risk and documented above. Net: **all P0 *code* corrective actions are closed and gated**; the
exit gate's only remaining P0 item is a green run of the new hosted CI workflow.
